...
const Login = () => {
  ...
  useEffect(() => {
    if (user) navigate(user.role === 'citizen' ? '/citizen-dashboard' : '/official-dashboard');
  }, [user]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Login failed', description: err instanceof Error ? err.message : 'Check your credentials.', variant: 'destructive' });
    ...
    <div className="min-h-screen bg-muted/30 flex items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold">Sign in to CivicConnect</h1>
          <p className="text-sm text-muted-foreground mt-1">Your community is waiting.</p>
        </div>
        ...
        <Card className="shadow-sm border-0">
          <CardContent className="pt-6 space-y-4">
            ...
            {GOOGLE_CLIENT_ID ? (
              ...
              <Button type="button" variant="outline" className="w-full h-11 font-medium gap-3" disabled>
                <svg className="h-4 w-4 shrink-0 opacity-40" viewBox="0 0 24 24">
